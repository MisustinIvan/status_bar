...
void setup_sections() {
    sections[0] = (Section){
        .timeout = 1, .updater = time_section_updater
    };

    sections[1] = (Section){
        .timeout = 1, .updater = volume_section_updater

    };

    sections[2] = (Section){
        .timeout = 60, .updater = battery_section_updater
    };
...
void setup() {
    sections = mmap(NULL, sizeof(Section)*NSECTIONS, PROT_READ|PROT_WRITE, MAP_SHARED|MAP_ANONYMOUS, -1, 0);
